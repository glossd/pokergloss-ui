...
* `src/ChangeAvatar.js` is the JavaScript entry point.
...
```js
import React, { Component } from 'react';
import './BigTextButton.css'; // Tell Webpack that Button.js uses these styles
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.css`. It would still be imported from `src/ChangeAvatar.js`, but you could always remove that import if you later migrate to a different build tool.
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your ```src/ChangeAvatar.js``` file:
...
* If the watcher doesn’t see a file called `ChangeAvatar.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebookincubator/create-react-app/issues/1164) due to a Webpack bug.
